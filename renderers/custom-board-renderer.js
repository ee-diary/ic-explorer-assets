...
var CustomBoardRenderer = (function() {
  ...
  function drawTeensy41(svg, config) {
    ...
    var chipX = Math.round((220 - chipW) / 2), chipY = Math.round((600 - chipH) / 2); // 40, 236
  ...
  function drawRaspberryPi3(svg, config) {
    // Placeholder - would contain full Raspberry Pi drawing
    drawGenericBoard(svg, config);
  }
  
  function drawArduinoNano(svg, config) {
    // Placeholder - would contain full Arduino Nano drawing
    drawGenericBoard(svg, config);
  }
  ...
  function attachPinInteractivity(svg, config) {
    ...
    chips.forEach(function(el) {
      ...
      var chip = config.components ? config.components.find(function(c) { return c.id === chipId; }) : null;
