...
window.QFPRenderer = {
  ...
  draw: function(svg, config) {
    ...
    dot.setAttribute('cx', -half + 15);
    dot.setAttribute('cy', -half + 15);
    ...
    // Counter-clockwise: LEFT (top→bottom) → BOTTOM (left→right) → RIGHT (bottom→top) → TOP (right→left)
    var pinPositions = [];
    var i;
    ...
    // ── Draw pins ─────────────────────────────────────────────────────────
    for (i = 0; i < pinPositions.length && i < pins.length; i++) {
      var pos  = pinPositions[i];
      var pin  = pins[i];
      var col  = this.getPinColor(pin.type);
      var side = pos.side;

      // Rect dimensions and position vary by side
      var rw, rh, rx, ry;
      if (side === 'left') {
        rw = pinLength; rh = pinWidth - pinGap;
        rx = -half - pinLength;   // extends left from body edge
        ry = pos.cy - rh / 2;
      } else if (side === 'right') {
        rw = pinLength; rh = pinWidth - pinGap;
        rx = half;                // extends right from body edge
        ry = pos.cy - rh / 2;
      } else if (side === 'bottom') {
        rw = pinWidth - pinGap; rh = pinLength;
        rx = pos.cx - rw / 2;
        ry = half;                // extends down from body edge
      } else { // top
        rw = pinWidth - pinGap; rh = pinLength;
        rx = pos.cx - rw / 2;
        ry = -half - pinLength;   // extends up from body edge
      }
